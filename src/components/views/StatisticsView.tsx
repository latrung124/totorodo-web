    ...
    <div className="flex-1 flex flex-col h-full bg-[#F3F4F6] rounded-tl-3xl overflow-hidden">
      ...
      <div className="bg-white px-8 py-6 border-b border-gray-100 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm">Track your productivity flow.</p>
        </div>
        <div className="flex bg-gray-100 p-1 rounded-xl">
          {(['time', 'streak', 'history'] as const).map(t => (
            ...
            >
              {t === 'time' ? 'Time Spent' : t === 'streak' ? 'Streak' : 'History'}
            </button>
          ))}
        </div>
      </div>
            ...
            <div className="col-span-12 lg:col-span-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-bold text-gray-800 flex items-center gap-2">
                  <Activity size={18}/> Time Usage vs Plan
                </h3>
                <select className="bg-gray-50 border border-gray-200 rounded-lg text-xs px-2 py-1 text-gray-600">
              ...
              <div className="flex justify-center gap-6 mt-4 text-xs">
                <div className="flex items-center gap-1">
                  <div className="w-3 h-0.5 bg-orange-500"></div> Actual
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-3 h-0.5 border-t border-gray-400 border-dashed"></div> Planned
                </div>
              </div>
            </div>
            ...
            <div className="col-span-12 lg:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-2 mt-6">
                {[
                  ...
                  { l: 'In Progress', c: 'bg-blue-500' },
                  { l: 'To Do', c: 'bg-amber-500' },
                  { l: 'Pending', c: 'bg-red-500' }
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs text-gray-600">
                    <div className={`w-2 h-2 rounded-full ${item.c}`}></div> {item.l}
                  </div>
                ))}
              </div>
            </div>
                ...
                {['Focus', 'Short', 'Long'].map((label, i) => (
                  <div key={i} className="flex flex-col items-center gap-2 w-16 group">
                    <div className="w-full flex items-end gap-1 h-full">
                      <div 
                        style={{height: i===0 ? '80%' : '40%'}} 
                        className="flex-1 bg-indigo-500 rounded-t-sm relative group-hover:opacity-90 transition-opacity"
                      />
                      <div 
                        style={{height: i===0 ? '20%' : '10%'}} 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
              <div className="w-24 h-24 bg-orange-50 rounded-full flex items-center justify-center mb-4 animate-bounce">
                <Flame size={48} className="text-orange-500" fill="#F97316" />
              </div>
              ...
              <div className="flex gap-8 text-left w-full max-w-xs bg-gray-50 p-4 rounded-xl">
                <div>
                  <p className="text-[10px] text-gray-400 uppercase font-bold">Longest</p>
                  <p className="text-lg font-bold text-gray-800">24 Days</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 uppercase font-bold">Started</p>
                  <p className="text-lg font-bold text-gray-800">Nov 01</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-xs font-bold text-gray-400">November 2025</span>
              </div>
              <CalendarGrid />
            </div>
          </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Pomodoros</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Time Spent</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Finished Date</th>
                  ...
                  <tr key={task.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1 text-sm font-medium text-gray-700">
                        <div className="w-2 h-2 bg-indigo-500 rounded-full"></div> {task.pomodoros}
                      </div>
                    </td>
